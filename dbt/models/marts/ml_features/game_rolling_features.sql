...
WITH cumulative AS (
    SELECT
       ...
       COUNT(*) OVER (PARTITION BY game_index ORDER BY DATE(timestamp_created) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS num_reviews,
       SUM(voted_up::int) OVER (
         PARTITION BY game_index ORDER BY DATE(timestamp_created) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
